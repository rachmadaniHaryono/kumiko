...
class Kumiko:
                ...
                if self.options["debug_dir"]:
                    for i in range(len(new)):
                        cv.drawContours(
                            ...
                            [new[n].polygon],
                            0,
                            self.subpanel_colours[i % len(self.subpanel_colours)],
                            contourSize,
                        )
    ...
    def parse_image(self, filename, url=None):
        ...
        for contour in contours:
            ...
            if (
                panel.w < infos["size"][0] * self.options["min_panel_size_ratio"]
                or panel.h < infos["size"][1] * self.options["min_panel_size_ratio"]
            ):
                continue
